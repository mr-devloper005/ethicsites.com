...
const FORM_CONFIG: Record<TaskKey, { title: string; description: string; fields: Field[] }> = {
  ...
  image: {
    title: "Create image",
    description: "Publish a gallery-style image post stored locally in your browser.",
    fields: [
  ...
  profile: {
    ...
    description: "Add a photographer or studio profile stored locally in your browser.",
    fields: [
      { key: "brandName", label: "Display name", type: "text", required: true },
      { key: "summary", label: "Short summary", type: "textarea", required: true },
      { key: "description", label: "About you", type: "textarea" },
      { key: "website", label: "Website URL", type: "url", required: true },
      { key: "logo", label: "Logo or headshot URL", type: "url", required: true },
    ],
  },
  social: {
...
export type CreateTaskFormProps = {
  taskKey: TaskKey;
  /** When false, the form is shown even if the task is disabled in site config (used by /create/image and /create/profile). */
  requireTaskEnabled?: boolean;
  /** Override back navigation; defaults to the task route from config, then home. */
  backHref?: string;
};

export function CreateTaskForm({ taskKey, requireTaskEnabled = true, backHref }: CreateTaskFormProps) {
  const { user } = useAuth();
  const { toast } = useToast();
  const router = useRouter();

  const taskDefinition = useMemo(
    () => SITE_CONFIG.tasks.find((task) => task.key === taskKey),
    [taskKey]
  );

  const taskConfig = useMemo(() => {
    if (!taskDefinition) return undefined;
    if (requireTaskEnabled && !taskDefinition.enabled) return undefined;
    return taskDefinition;
  }, [taskDefinition, requireTaskEnabled]);

  const formConfig = FORM_CONFIG[taskKey];
  const resolvedBackHref = backHref ?? taskDefinition?.route ?? "/";
